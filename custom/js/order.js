...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
	   	...
	   	// Pack value array validation
	   	var pack = document.getElementsByName('pack[]');		   	
	   	var validatePack;
	   	for (var x = 0; x < pack.length; x++) {       
	 			var packId = pack[x].id;
		    if(pack[x].value == ''){	    	
		    	$("#"+packId+"").after('<p class="text-danger"> Pack is required!! </p>');
		    	$("#"+packId+"").closest('.form-group').addClass('has-error');	    		    		    	
	      } else {      	
		    	$("#"+packId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
	      } 
	   	}  // for
	   	...
	   	for (var x = 0; x < batch.length; x++) {       
	 			var batchId = batch[x].id;
		    if(batch[x].value == ''){	    	
		    	$("#"+batchId+"").after('<p class="text-danger"> Batch Field is required!! </p>');
		    	$("#"+batchId+"").closest('.form-group').addClass('has-error');	    		    		    	
	      } else {      	
		    	$("#"+batchId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
	      } 
	   	}  // for
	   	...
	   	// expDateate array validation

	   	var expDate = document.getElementsByName('expDate[]');		   	
	   	var validateExpDate;
	   	for (var x = 0; x < expDate.length; x++) {       
	 			var expDateId = expDate[x].id;
		    if(expDate[x].value == ''){	    	
		    	$("#"+expDateId+"").after('<p class="text-danger"> Exp required!! </p>');
		    	$("#"+expDateId+"").closest('.form-group').addClass('has-error');	    		    		    	
	      } else {      	
		    	$("#"+expDateId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
	      } 
	   	}  // for
	   	...
	   	for (var x = 0; x < expDate.length; x++) {       						
		    if(expDate[x].value){	    		    		    	
	   		...
	   		// MRP array validation

	   	var mrp = document.getElementsByName('mrp[]');		   	
	   	var validateMrp;
	   	for (var x = 0; x < mrp.length; x++) {       
	 			var mrpId = mrp[x].id;
		    if(mrp[x].value == ''){	    	
		    	$("#"+mrpId+"").after('<p class="text-danger"> MRP required!! </p>');
		    	$("#"+mrpId+"").closest('.form-group').addClass('has-error');	    		    		    	
	      } else {      	
		    	$("#"+mrpId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
	      } 
	   	}  // for
		...
		// gst value array validation
	   	var gstValue = document.getElementsByName('gstValue[]');		   	
	   	var validateGst;
	   	for (var x = 0; x < gstValue.length; x++) {       
	 			var gstValueId = gstValue[x].id;
		    if(gstValue[x].value == ''){	    	
		    	$("#"+gstValueId+"").after('<p class="text-danger"> GST Field is required!! </p>');
		    	$("#"+gstValueId+"").closest('.form-group').addClass('has-error');	    		    		    	
	      } else {      	
		    	$("#"+gstValueId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
	      } 
	   	}  // for
	   	...
	   	// indivisual product discount value array validation
	   	var pdisValue = document.getElementsByName('pdisValue[]');		   	
	   	var validatepdisValue;
	   	for (var x = 0; x < pdisValue.length; x++) {       
	 			var pdisValueId = pdisValue[x].id;
		    if(pdisValue[x].value == ''){	    	
		    	$("#"+pdisValueId+"").after('<p class="text-danger"> Dis Field is required!! </p>');
		    	$("#"+pdisValueId+"").closest('.form-group').addClass('has-error');	    		    		    	
	      } else {      	
		    	$("#"+pdisValueId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
	      } 
	   	}  // for

	   	for (var x = 0; x < pdisValue.length; x++) {       						
		    if(pdisValue[x].value){	    		    		    	
		    	validatepdisValue = true;
	      } else {      	
		    	validatepdisValue = false;
	      }          
	   	} // for       	
...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		var mywindow = window.open('', 'Stock Management System',  '_blank','height=400,width=600');
        mywindow.document.write('<html><head><title>Order Invoice</title>'); 
        mywindow.document.write("<link rel=\"stylesheet\" type=\"text/css\" href=\"assests/bootstrap/css/bootstrap.min.css\" />");       
		...
		success:function(response) {
			...
			tr += '</select>'+
				...
				'<td style="padding-left:10px;">'+
			  				...
			  				'<td style="padding-left:10px;">'+
			  					'<div class="form-group">'+
			  					'<input type="number" name="free[]" id="free'+count+'" onkeyup="" autocomplete="off" class="form-control" min="0" value="0" />'+
			  					'</div>'+
			  				'</td>'+
			  				'<td style="padding-left:10px;">'+
			  					'<div class="form-group">'+
			  					'<input type="text" name="pack[]" id="pack'+count+'" onkeyup="" autocomplete="off" class="form-control" min="1" />'+
			  					'</div>'+
			  				'</td>'+
			  				'<td style="padding-left:10px;">'+
			  					'<div class="form-group">'+
			  					'<input type="text" name="batch[]" id="batch'+count+'" onkeyup="" autocomplete="off" class="form-control" min="1" />'+
			  					'</div>'+
			  				'</td>'+
			  				'<td style="padding-left:10px;">'+			  					
			  					'<input type="text" name="rate[]" id="rate'+count+'" autocomplete="off" disabled="true" class="form-control" />	'+		  					
			  				...
			  				'<td style="padding-left:10px;">'+
			  					'<div class="form-group">'+
			  					'<input type="text" name="expDate[]" id="expDate'+count+'" onkeyup="" autocomplete="off" class="form-control" min="1" />'+
			  					'</div>'+
			  				'</td>'+
			  				'<td style="padding-left:10px;">'+
			  					'<div class="form-group">'+
			  					'<input type="number" name="mrp[]" id="mrp'+count+'" onkeyup="" autocomplete="off" class="form-control" min="1" disabled="true" />'+
			  					'</div>'+
			  				'</td>'+
			  				'<td style="padding-left:10px;">'+
			  					'<div class="form-group">'+
			  					'<input type="number" name="gstValue[]" id="gstValue'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="1" />'+
			  					'</div>'+
			  				'</td>'+
			  				'<td style="padding-left:10px;">'+
			  					'<div class="form-group">'+
			  					'<input type="number" name="pdisValue[]" id="pdisValue'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="1" />'+
			  					'</div>'+
			  				'</td>'+
			  				'<td style="padding-left:10px;">'+			  					
			  					'<input type="text" name="total[]" id="total'+count+'" autocomplete="off" class="form-control" disabled="true" />'+			  					
			  				...
			  				'<td style="padding-left:10px;">'+
			  					'<div class="form-group">'+
			  					'<input type="text" name="netamount[]" id="netamount'+count+'" onkeyup="" autocomplete="off" class="form-control" min="1" />'+
			  					'</div>'+
			  				'</td>'+
			  				'<td>'+
					'<button class="btn btn-default removeProductRowBtn" type="button" onclick="removeProductRow('+count+')"><i class="glyphicon glyphicon-trash"></i></button>'+
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					//GST
					$("#gstValue"+row).val(0);
					var gstvalueCal = parseFloat(total);
					// var Sgst_Cgst_value=gstvalueCal*2;
					$("#netamount"+row).val(gstvalueCal.toFixed(2));



					//Indivitual Product discount
					$("#pdisValue"+row).val(0);
...
function getTotal(row = null) {
	if(row) {
		...
		var getGstvalue=$("#gstValue"+row).val() *2 /100;
		var net_amount = total + (total * getGstvalue) ;
		$("#netamount"+row).val(net_amount.toFixed(2));


		//calculate SGST and CGST 
		var pdisCal=($("#pdisValue"+row).val() / 100).toFixed(2);
		var mult = net_amount * pdisCal;
		var discont = net_amount - mult;
		$("#netamount"+row).val(discont.toFixed(2));
...
function subAmount() {
	...
	for(x = 0; x < tableProductLength; x++) {
		...
		totalSubAmount = Number(totalSubAmount) + Number($("#netamount"+count).val());
	} // /for
...
{
	var customerPhone=$("#clientContact").val();
		// alert(customerPhone);

	$.ajax({
            url: 'php_action/fetchCustomerdata-in-order.php',
            type: 'POST',
